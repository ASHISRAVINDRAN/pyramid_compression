...
from Load_Save_Model_GD import save_model_stage1
from Load_Save_Model_GD import save_model
from LoadModel import VGGNetNthLayer
from Train_2stages_GD import Train_stage1_GD
from Train_2stages_GD import Train_stage2
import torch.optim
...
'''
model_name='SVGG14_BACKWARD'
print('model='+model_name)
if model_name=='SVGG17_BACKWARD':
    num_opt=1
elif model_name=='SVGG14_BACKWARD':
    num_opt=2
elif model_name=='SVGG11_BACKWARD':
    num_opt=3
elif model_name=='SVGG8_BACKWARD':
    num_opt=4
...
if model_name=='VGG19':
    optimizer=torch.optim.Adam([{'params': model.features.parameters()},
                {'params': model.classifier.parameters(), 'lr': 1e-3}], lr=1e-4, betas=(0.9, 0.999), eps=1e-08,weight_decay=0.005)
else:
...
for fnum in range(num_opt):
    optimizer_str='optimizer.append(torch.optim.Adam('+parameters_str+str(fnum+1)+'.parameters(), lr=lr_stage1, betas=(0.9, 0.999), eps=1e-08,weight_decay=0.0005))'
    exec(optimizer_str)   
    scheduler.append(ReduceLROnPlateau(optimizer[fnum], 'min',verbose=True,patience=5,eps=1e-9,threshold=1e-20,threshold_mode='abs'))
                               
print ('STAGE1')
trainloss_to_file,testloss_to_file,Parameters=Train_stage1_GD(model,optimizer,Trainloader,Testloader,epochs=None,Train_mode=train_mode,
...
trainAcc_to_file,testAcc_to_file,_,_,_=Train_stage2(model,optimizer[0],Trainloader,Testloader, epochs=0,
                                                                                                    Train_mode=train_mode,
                                                                                                    Model_name=model_name,
                                                                                                    Dataset=dataset,
